    ...
    <div className="px-4 py-6 md:px-8 md:py-10 max-w-3xl mx-auto">
      ...
      <header className="flex items-center gap-4 mb-8">
        <div className="relative group">
          ...
          >
            {uploadingAvatar ? (
              ...
              <div className="w-full h-full flex items-center justify-center">
                <UserIcon size={32} className="text-juju-texte-mute" />
              </div>
            )}
          </button>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-juju-texte-mute dark:text-juju-texte-mute truncate">
            {user?.email}
          </p>
        </div>
      </header>
        ...
        <>
          ...
          {stats.articles.par_categorie.length > 0 && (
            <section className="mb-8">
              <h2 className="text-xs uppercase tracking-widest text-juju-texte-mute mb-3">
      ...
      <section className="mb-6">
        ...
        <div className="space-y-3">
          {/* Thème */}
          <div className="p-4 bg-juju-bleu/10 dark:bg-juju-bleu border border-juju-bordure rounded-lg">
      ...
      <section>
        <button
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 px-6 py-3 border border-juju-bordure text-juju-texte-mute rounded-lg hover:border-red-500 hover:text-red-400 transition-colors"
        >
          <LogOut size={18} />
          Se déconnecter
        </button>
...
function StatCard({ icon: Icon, value, label, sublabel, valueClass }) {
  return (
    <div className="p-4 bg-juju-bleu/10 dark:bg-juju-bleu border border-juju-bordure rounded-lg text-center">
      <Icon size={18} className="mx-auto mb-2 text-juju-dore" />
      <p className={`font-medium ${valueClass || 'text-2xl md:text-3xl'}`}>{value}</p>
      <p className="text-[10px] uppercase tracking-wider text-juju-texte-mute mt-1">{label}</p>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 w-32">
        ...
        <span className="text-xs text-juju-texte-mute w-6 text-right">{count}</span>
      </div>
    </div>
  );
}
...
function InfoRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-center gap-3 p-4 bg-juju-bleu/10 dark:bg-juju-bleu border border-juju-bordure rounded-lg">
      <Icon size={18} className="text-juju-dore" />
      <div className="flex-1 min-w-0">
        <p className="text-xs text-juju-texte-mute">{label}</p>
    ...
    <button
      ...
      className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
        isActive
          ...
          : 'bg-transparent text-juju-texte border-juju-bordure hover:border-juju-texte-mute'
      }`}
    >
      {Icon && <Icon size={14} />}
      {label}
